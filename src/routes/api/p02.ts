...
export const APIRoute = createAPIFileRoute("/api/p02")({
  POST: async ({ request }) => {
    try {
      ...
      if (body.action === "speech") {
        if (!body.sessionId || !body.text) {
          return Response.json(
            { ok: false, error: "sessionId and text required" },
            { status: 400 },
          );
        }
        const result = await VoiceDevelopmentService.processUserSpeech(
          body.sessionId,
          body.text,
        );
